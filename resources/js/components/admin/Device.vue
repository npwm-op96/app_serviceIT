    ...
    <v-main>
        <v-container>
            <v-row>
                ...
                <v-col cols="10" sm="" md="3" lg="3" xs="6">
                    <v-text-field
                        label="ค้นหา"
                        prepend-icon="search"
                        v-model="Search_emp"
                ...
                <v-col cols="10" sm="3" md="3" lg="3" xs="6">
                    <v-menu
                        top
                        v-model="start_time"
        ...
        <v-container class="">
            ...
            <br />
            <v-row v-for="GetDevice in GetDevices" :key="GetDevice.id">
                <v-col cols="12" sm="1" md="1" lg="1">
                    {{ GetDevice.id }}
                </v-col>
                <v-col cols="12" sm="2" md="2">
                    {{ GetDevice.employee.name }}
                </v-col>
                <v-col cols="12" sm="2" md="2">
                    {{ GetDevice.product.name }}
                </v-col>
                <v-col cols="12" sm="2" md="2">
                              <v-chip v-if="GetDevice.status=='active'"
                            class="ma-2"
                            color="success"
                            text-color="white"
                            >
                        ...
                        arrow_circle_up
                                       ...
                                       <v-chip  v-else-if="GetDevice.status=='inactive'"
                            class="ma-2"
                            color="error"
                            text-color="white"
                            >
...
></v-pagination> -->
        ...
        <v-row>       
            ...
            <v-col cols="1" sm="1" md="1" lg="1" xs="1">
                <v-select
                    :items="page"
                    label="Page"
                    v-model="per_page"
...
export default {
    ...
    created() {
        // this.link = this.$router.history.current.path;

        //  this.link = this.$router.map() ;
        for (var i = 0; i < 10; i++) {
            this.page.push(i * 5);
        }
        ...
        // this.interval = setInterval(() => {
            ...
            if (Search_emp != "") {
                ...
                axios
                    ...
                    .then(response => {
                        this.GetDevices = response.data.data;
    ...
    methods: {
        ...
        create_emp() {
            ...
            console.log(in_emp);
            axios.post("/api/CreateUsers", in_emp).then;
